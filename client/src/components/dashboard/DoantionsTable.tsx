...
const getDonationTypeBadge = (type: Donation['type']) => {
	const base =
		'flex items-center px-2 py-1 rounded-sm text-xs font-medium border w-fit';
...
const DonationsTable: React.FC = () => {
	...
	const handleViewDonation = (donation: Donation) => {
		console.log('View donation:', donation.id);
		// Could open a modal or navigate to details page
		showToast.message(`Viewing donation ${donation.id}`);
	};
					...
					<div className="flex flex-wrap gap-3 items-center">
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="start" className="w-48">
								...
								<DropdownMenuItem
									onClick={() => handleTypeFilterChange('ONE_TIME')}
									className={`cursor-pointer ${
										typeFilter === 'ONE_TIME'
											? 'bg-gray-100 text-gray-900'
											: 'text-gray-700'
								...
								<DropdownMenuItem
									onClick={() => handleTypeFilterChange('ANNUAL')}
									className={`cursor-pointer ${
										typeFilter === 'ANNUAL'
											? 'bg-gray-100 text-gray-900'
											: 'text-gray-700'
